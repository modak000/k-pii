...
A) **한국 공공 데이터 추가 평가** — *사용자 로컬* 에서만 가능 (클라우드 환경
   ...
   **2026-05-21 로컬 부트스트랩 (commit e02f135):** Wikipedia API 로 11편 ≈25K
   chars 코퍼스 → fp_collector → 46 후보 → 사람 검토 → common_words +50.
   같은 코퍼스에서 freq 2+ 후보 46 → 3 (-93%). 다음 반복은 실제 판결서/공문서
   데이터로 수행 권장 (Wikipedia 는 어휘 폭이 좁음).
