...
### D-027. AI Hub 데이터셋 접근 (정정 — 안심존 아님, 로그인+활용승인)
**정정 사항:** 본 항목의 이전 버전은 AI Hub 71844 / 569 를 "안심존" 으로
오인 기재했음 (페이지의 사이드 메뉴에 "안심존이란?" 링크가 항상 보여서 혼동).
실제로는 두 데이터셋 모두 *3. 개방데이터* — 즉 *원격 다운로드 가능* 하지만
다음을 거쳐야 함:
  1. AI Hub 계정 가입 (무료)
  2. 데이터셋 페이지에서 "데이터 다운로드 신청" → 활용 목적 기입 → *승인*
     (자동 또는 수동 검토, 보통 1~2 영업일)
  3. 승인 후 *AI 허브 Shell* 클라이언트 또는 *AI 허브 오픈 API* 로 받음
     (분할 압축 .zip.part* → Linux/WSL 에서 `find ... | xargs cat` 으로 병합)

**조사한 데이터셋들 (2026-05-21 사용자 페이지 제공):**

| dataSetSn | 이름 | 용량 | 도메인 적합도 |
|-----------|------|------|--------------|
| 71844 | 민간 민원 상담 LLM 사전학습 및 Instruction Tuning | 208.81 MB | 부분 (LG U+/하나카드/액티벤처 — 민간) |
| 71845 (24번) | 공공 민원 상담 LLM 사전학습 및 Instruction Tuning | 192.19 MB | **★ 정확히 부합 (중앙·지방행정기관)** |
| ?? (25번) | 금융분야 고객상담 | 306.91 MB | 부수 (은행/보험/증권) |
| 569 | 행정 문서 대상 기계독해 | 259.17 MB | **★ 정확히 부합 (행정문서 QA)** |

**Probe 결과:**
- /file/down.do 의 *부속 매뉴얼 zip* 만 무인증 다운로드 가능 (`dataMnlFileSn`,
  `cnstcPrcuseFileSn`, `aiModelMnlFileSn` — 모두 설치 가이드/사용 매뉴얼).
- 실제 학습 데이터의 file key 들 (553214~553237 / 32653~32676 등) 은 페이지
  HTML 에 노출되지만 그 endpoint 는 *세션 인증 + 활용 승인 상태 체크* 함.
- "샘플(경량) 데이터" 버튼은 `LoginLink()` 함수 호출 — 로그인 필수.
- 71844 페이지는 `smpleFileSn=<null>` — 샘플 파일 자체가 없음.

**casenote.kr / law.go.kr:** 별도 벽 — casenote 는 pro 계정 paywall (무료는
메타데이터만), law.go.kr 는 JavaScript 렌더링 (초기 HTML 에 본문 없음).

**결론 — 본 라이브러리 부트스트랩의 현실적 경로 (우선순위):**
  1. **★ 사용자 AI Hub 계정으로 71845 (공공 민원 상담) 활용승인 + Shell/API
     로 다운로드** → 사용자가 직접 받아서 `data/corpus/` 에 떨궈주시면
     fp_collector 가 받아서 처리. 라이브러리 자체에는 자격증명 저장 X.
  2. korea.kr 정책브리핑 추가 크롤링 (현재 20편 → 100편+; 키워드/카테고리별)
  3. Korean Wikisource 위키문헌 (`ko.wikisource.org`) — 법령 본문 plain Wikitext
  4. 국가법령정보 OPEN API (open.law.go.kr) — 키 등록 후 사용
