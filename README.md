...
> - 합성 풍부화 코퍼스 F1 = 0.877 (회귀 감지)
> - KDPII 일상 대화 (참고): F1 = 0.655 — *대화체 도메인, 1-2자 별명 50% 포함, 우리 도메인 지표 아님*
...
| 벤치마크 | 데이터 | 문서 수 | micro F1 | 도메인 적합도 |
|---|---|---:|---:|---|
| **공공 문서 본문 산문** (실측) | 가공 공문서 | 12 케이스 | **~0.83** | **사용자 메인 도메인** |
| 합성 코퍼스 (sanity) | 6 템플릿 다단락 | 60×5 | 0.877 | 회귀 감지 |
| [KDPII](https://ieeexplore.ieee.org/document/10681073) | 한국어 일상 대화 | 53,778 | 0.655 | 대화체 (참고용) |
| [KLUE-NER](https://github.com/KLUE-benchmark/KLUE) | 신문기사 PS | 5,000 | 0.322 | 자연어 어림짐작 |

⚠ **KDPII micro F1 0.655 가 사용자 도메인의 실질 성능이 아님.** KDPII PERSON
gold 의 50% 가 1-2자 별명·이름 단독 ("재명/미선/마미") — 일상 대화 도메인 형식.
공공 문서는 풀네임 위주로 우리 정책 (풀네임 검출) 적합. 자세한 도메인 분석은
[`docs/EVALUATION_REPORT.md`](docs/EVALUATION_REPORT.md) §5-G/H/I.

### 카테고리별 Tier (KDPII 기준 — 참고용)
...
| **D** | <0.20 | PERSON (0.175), CARD (0.130) | **도메인 한계 — 아래 설명** |

**PERSON 카테고리 — 도메인별 실질 성능:**

| 도메인 | PERSON F1 | 결론 |
|---|---:|---|
| **공공 문서 본문 산문** (사용자 메인) | **0.83+** | **운영 가능** |
| 합성 양식 + 본문 | ~0.88 | 좋음 |
| KDPII 대화체 (1-2자 50%) | 0.175 | 룰 기반 한계 |

→ KDPII PERSON gold 의 50% 가 별명/2자 이름 단독 ("재명/미선") 인데
